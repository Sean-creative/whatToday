...
<!--  번개 모임 개설 -->
<section id="wrap"
	style="border: 1px solid; margin-top: 30px; padding: 30px;">
	<form action="/thunder/list" method="get"
		...
		<!-- 상세검색창 -->
		<div
			style="border: 1px solid black; padding: 20px; background-color: lightgray;">
			...
			<div
				...
				<select name='category' id='category'
					style="width: 220px; height: 40px; font-size: 20px;"></select>

				<!-- DB의 컬럼명이 subclass에서 subcat으로 바뀌면서, 코드내에 Criteria를 사용하는 곳은 다 바꿔야함 -->
				<select name='subclass' id='subcat'
					style="width: 220px; height: 40px; font-size: 20px;"></select> <select
					name='city' id='city'
					style="width: 220px; height: 40px; font-size: 20px;" id="city"></select>
			...
			<div>
				<div>
					<input type="text" name='keyword'
						style="width: 600px; height: 30px; font-size: 20px;"
		...
		<div style="padding: 20px;">
			...
			<div
				...
				<c:forEach items="${list}" var="thunderItem">
					<div
						style="border: 1px solid black; width: 170px; height: 140px; cursor: pointer;"
	...
	<form id='actionForm' action="/thunder/list" method='get'>
		...
		<input type='hidden' name='category'
			value='<c:out value="${pageMaker.cri.category}"/>'> <input
			type='hidden' name='subclass'
			value='<c:out value="${pageMaker.cri.subclass}"/>'> <input
			type='hidden' name='city'
			value='<c:out value="${pageMaker.cri.city}"/>'> <input
			type='hidden' name='district'
			value='<c:out value="${pageMaker.cri.district}"/>'> <input
